def unweighted_mlp_model():
    ...
    for i in range(4):
        ...
        mlp.fit(x_train, y_train, validation_split=0.3, epochs=50, callbacks=[early_stopping, reduce_plateau])

        y_pred = mlp.predict(x_test)
        y_pred_bin = (y_pred > 0.5).astype(int)

        mlp.save(f"all_models/mlp_unweighted_{lbs[i]}.keras")
        models.append(f"all_models/mlp_unweighted_{lbs[i]}.keras")
